...
var StepBox=React.createClass({
	...
	handleTabHeadClick:function(i){
		this.refs["content"+i].handleStepContentToggle();
		$(this.refs["head"+i]).addClass("active").siblings().removeClass("active");
	},
	render:function(){
		var tabTags=[],tabContents=[];
		
		this.props.data.Children.forEach(function(item,i){
			tabTags.push(<div ref={"head"+i} className={this.checkTabTagIndex(i)} onClick={this.handleTabHeadClick.bind(this,i)} key={item.Id} >{item.Name} {item.Alias}</div>)
			tabContents.push(<StepContent ref={"content"+i}  BacteriaIndex={this.props.BacteriaIndex} DayIndex={this.props.DayIndex} className={this.checkTabContentIndex(i) }  item={item} key={item.Id}/>);
...
var StepContent=React.createClass({
	...
	handleStepContentToggle:function(){
		$(this.refs.stepContent).fadeIn().siblings().hide();
	},
	checkTabTagIndex:function(i){
	...
	render:function(){
		...
		return(
			<div className={this.props.className} ref="stepContent">
	...
	render:function(){
			...
			this.state.Children.forEach(function(item,i) {
		     	switch(item.ControlType)
		     	{
		     		...
		     		case 1000:childrens.push(<VirticalItem     item={item} key={item.Id}/>);break;
		     		case 1005:childrens.push(<VirticalItem     item={item} key={item.Id}/>);break;
